...
namespace Framework
{
	...
	void Renderer::Submit(const std::shared_ptr<Shader> a_shader, const std::shared_ptr<VertexArray> a_vertexArray, std::vector<std::string> a_textures, const glm::mat4 & a_transform)
	{
		a_shader->Bind();
		a_shader->UploadUniformMat4("u_ProjectionView", m_sceneData->ProjectionViewMatrix);
		a_shader->UploadUniformMat4("u_ObjectMatrix", a_transform);
		a_shader->UploadUniformVec3("u_ViewPos", m_sceneData->ProjectionViewMatrix[3].xyz);

		for (size_t i = 0; i < a_textures.size(); i++)
		{
			auto texture = Application::Get().GetTextureLibrary().GetTexture(a_textures[i]);
			a_shader->UploadTexture(std::string(texture->TextureTypeToString(texture->GetType())).append("Texture"), texture, i);
		}
		
		for (int i = 0; i < m_sceneData->m_dirLights.size(); i++)
		{
			a_shader->UploadUniformVec3(std::string("u_DirLight").append(std::to_string(i)), *m_sceneData->m_dirLights[i]);
		}
