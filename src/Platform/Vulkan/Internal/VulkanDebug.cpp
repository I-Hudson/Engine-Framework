...
namespace Framework
{
	namespace Vulkan
	{
		...
		VkResult  VulkanDebug::SetupDebugger(VkInstance instance, const VkAllocationCallbacks* allocator)
		{
			...
			if (func != nullptr)
			{
				return func(instance, &createInfo, allocator, &m_debugMessager);
			}
			else
			{
		...
		VkDebugUtilsMessengerCreateInfoEXT VulkanDebug::CreateDebugMessengerInfo()
		{
		...
		void VulkanDebug::Destroy(VkInstance instance)
		{
			...
			if (func != nullptr) 
			{
				func(instance, m_debugMessager, nullptr);
			}
		}
